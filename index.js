...
controller.hears(
    ...
    controller.storage.users.get(userid, function(err,user_data){
        var temp = user_data;
        // console.log("temp.user.username: " + temp.user.username);
        // console.log("props: " + Object.getOwnPropertyNames(temp.user));
        if (temp===undefined){
...
enter = function(res, convo){
    ...
    if (!user.knownPlayer){
        convo.ask("The Innkeeper grunts. \"Well met, " + user.username + ". Haven't seen you around here before. You mean to introduce yourself, and begin your adventure in Coneshire?\"", [
        {
...
instructions = function(res,convo){
    ...
    if (temp.includes('village')){
        ...
        convo.ask("What can I answer next?", function(res,convo){
            instructions(res,convo);
            convo.next();
        });
