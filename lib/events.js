...
module.exports = {
	...
	eventReturner: function(x){
		...
		for (var prop in alltheevents) {
			console.log("events for loop");
			if (prop===x){
				console.log("found prop");
				var blerg = alltheevents[prop].replace(/currentuser/, currentuser);
				console.log("blerg: " + blerg);
				// var blerg;
				// if (prop==="place0" || prop==="place1" || prop==="place2" || prop==="place3" || prop==="magic") {
				// 	// those vars
				// 	blerg = alltheevents[prop];
				// } else if (prop==="newplayer") {
				// 	blerg = alltheevents[prop];
				// } else {
				// 	blerg = alltheevents[prop];
				// }
